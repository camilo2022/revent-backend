...
<title>Comprobante de cruce de saldos - REVENT CALZADO S.A.S.</title>
<!--[if mso]>
<noscript>
<xml>
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:640px; width:100%;">
                ...
                <tr>
                    <td style="background-color:#111827; border-radius:12px; padding:26px 24px; font-family:Segoe UI, Arial, sans-serif;">
                        ...
                        <p style="margin:0; font-size:19px; font-weight:bold; color:#ffffff; line-height:1.3;">
                            Comprobante de cruce contable {{ $voucher['name'] ?? '' }}
                        </p>
                    </td>
                </tr>
                ...
                <tr>
                    <td style="background-color:#ffffff; border:1px solid #eef0f2; border-radius:12px; padding:24px; font-family:Segoe UI, Arial, sans-serif;">
                        ...
                        <p style="margin:0 0 16px 0; font-size:14px; color:#374151; line-height:1.6;">
                            A continuación, le remitimos detalle del cruce contable
                            @if(!empty($voucher_id))
                                <a href="https://siigonube.siigo.com/#/journal-entry/1569/{{ $voucher_id }}" target="_blank" style="color:#1d4ed8; font-weight:bold; text-decoration:none;">
                                    {{ $voucher['name'] ?? '' }}
                                </a>
                            @else
                ...
                <tr><td style="height:16px; line-height:16px; font-size:0;">&nbsp;</td></tr>
                @php
                    $date = now('America/Bogota')->format('d/m/Y H:i') . ' hrs.';
                @endphp
                <!-- Detalle de recibos de pago (anticipos) aplicados -->
                <tr>
                    <td style="background-color:#ffffff; border:1px solid #eef0f2; border-radius:12px; padding:16px 20px 8px 20px; font-family:Segoe UI, Arial, sans-serif;">
                        ...
                        <p style="margin:0 0 10px 0; font-size:12px; font-weight:bold; color:#111827; text-transform:uppercase; letter-spacing:0.3px;">
                            Recibos de pago (anticipos) aplicados
                        </p>

                        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">

                            @php $totalUsadoRecibos = 0; @endphp

                            @foreach ($recibos as $item)
                            @php
                                $doc = $item['doc'] ?? [];
                                $saldo = $item['saldo'] ?? abs($doc['Saldo'] ?? 0);
                                $usado = $item['used'] ?? 0;
                                $disponible = max($saldo - $usado, 0);
                                $totalUsadoRecibos += $usado;

                                $fechaRaw = $doc['DueDate'] ?? null;
                                $fechaCorta = $fechaRaw ? \Carbon\Carbon::parse($fechaRaw)->format('d/m/y') : '-';

                                $reciboDoc = $doc['DocName'] ?? '-';

                                $usadoCompleto = $disponible <= 0;
                                $badgeBg = $usadoCompleto ? '#dcfce7' : '#ffedd5';
                                $badgeColor = $usadoCompleto ? '#166534' : '#9a3412';
                            @endphp
                            <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td align="right" valign="middle" style="white-space:nowrap; padding-left:12px;">
                                                <span style="display:inline-block; background-color:{{ $badgeBg }}; color:{{ $badgeColor }}; font-size:12px; font-weight:bold; font-family:Segoe UI, Arial, sans-serif; padding:5px 10px; border-radius:999px; white-space:nowrap;">
                                                    ${{ number_format($usado, 0, ',', '.') }}
                                                </span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="2" style="padding-top:4px; font-family:Segoe UI, Arial, sans-serif; font-size:11px; color:#6b7280;">
                                                Valor anticipo: <strong style="color:#374151;">${{ number_format($saldo, 0, ',', '.') }}</strong>
                                                &middot; Usado: <strong style="color:#166534;">${{ number_format($usado, 0, ',', '.') }}</strong>
                                                &middot; Disponible restante:
                                                <strong style="color: {{ $disponible > 0 ? '#166534' : '#6b7280' }};">
                                    ...
                                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                                        <tr>
                                            ...
                                            <td align="right" valign="middle" style="font-family:Segoe UI, Arial, sans-serif; font-size:13px; color:#111827; font-weight:bold; white-space:nowrap; padding-left:12px;">
                                                ${{ number_format($totalUsadoRecibos, 0, ',', '.') }}
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                        ...
                        <p style="margin:4px 0 8px 0; font-size:12px; color:#166534; text-align:left;">
                            Total usado de recibos de pago el día {{ $date }}
                        </p>
                ...
                <!-- Detalle de documentos (facturas) cubiertos por el cruce -->
                <tr>
                    <td style="background-color:#ffffff; border:1px solid #eef0f2; border-radius:12px; padding:16px 20px 8px 20px; font-family:Segoe UI, Arial, sans-serif;">
                                ...
                                <td style="padding:11px 0; border-bottom:1px solid #f1f3f5;">
                                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                                        <tr>
                                            <td valign="middle" style="font-family:Segoe UI, Arial, sans-serif;">
                                                <span style="font-size:13px; font-weight:bold; color:#111827;">
                                                    {{ $doc['DueName'] ?? '' }} - {{ $bodega }} - ({{ $docNumero }})
                                                </span>
                                                <div style="font-size:11px; color:#9ca3af; margin-top:2px;">
                                                    {{ $fechaCorta }} &middot; O.C.: {{ $ordenCompra }} &middot; Pares: {{ $pares }}
                                                </div>
                                            </td>
                                            <td align="right" valign="middle" style="white-space:nowrap; padding-left:12px;">
                                                <span style="display:inline-block; background-color:{{ $badgeBg }}; color:{{ $badgeColor }}; font-size:12px; font-weight:bold; font-family:Segoe UI, Arial, sans-serif; padding:5px 10px; border-radius:999px; white-space:nowrap;">
                        ...
                        <p style="margin:4px 0 8px 0; font-size:12px; color:#166534; text-align:left;">
                            Total aplicado a documentos el día {{ $date }}
                        </p>
