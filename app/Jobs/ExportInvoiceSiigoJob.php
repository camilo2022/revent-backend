...
use App\Mail\ExportInvoiceSiigoMail;
...
class ExportInvoiceSiigoJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $filename = 'reports/facturas_de_venta_' . now()->format('Y_m_d_His') . '.xlsx';
        ...
        Mail::to($this->notifyEmail)->send(new ExportInvoiceSiigoMail(success: true, filename: $filename, downloadUrl: $downloadUrl));
    }

    public function failed(\Throwable $e): void
    {
        Mail::to($this->notifyEmail)->send(new ExportInvoiceSiigoMail(success: false, errorMessage: $e->getMessage()));
    }
    ...
    private function sellers_siigo(string $token, array $filters = []): void
    {
        ...
        $this->sellers = collect($this->sellers)->keyBy('id')->all();
    }
    ...
    private function invoices_siigo(string $token, array $filters = []): void
    {
        ...
        do {
            ...
            if (!empty($data['results'])) {
                $this->invoices = array_merge($this->invoices, $data['results']);
            }
    ...
    private function credit_notes_siigo(string $token, array $filters = []): void
    {
        $allowedFilters = ['page', 'page_size', 'created_start', 'created_end'];
        ...
        do {
            ...
            if (!empty($data['results'])) {
                $this->credit_notes = array_merge($this->credit_notes, $data['results']);
            }
